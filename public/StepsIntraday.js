...
function getUserActivityData(date) {
  ...
  fetch(createFitbitRequest(date), init)
    ...
    .then(function(activityData) {
      userActivityData = activityData["activities-steps-intraday"].dataset;
...
function createInterdayGraph(activityData) {
  var xValues = [];
  var yValues = [];

  activityData.forEach(function(data, idx, arr) {
    if (detailLevel === "1min") {
      if (
        idx !== arr.length - 1 &&
